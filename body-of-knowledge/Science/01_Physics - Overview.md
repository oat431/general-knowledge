---
tags: [overview, physics, math-sci, high-school, ipst]
---
...
```mermaid
flowchart TD
    MEAS["Measurement"] --> MOTION["Kinematics"]
    MOTION --> NEWTON["Dynamics"]
    NEWTON --> WORK["Work & Energy"]
    NEWTON --> MOMENTUM["Momentum"]
    WORK --> ROTATION["Rotational Motion"]
    MOMENTUM --> ROTATION
    NEWTON --> OSC["Oscillations"]
    OSC --> WAVES["Waves"]
    WAVES --> SOUND["Sound"]
    WAVES --> WAVEOPTICS["Wave Optics"]
    WORK --> THERMO["Thermodynamics"]
    NEWTON --> ESTAT["Electrostatics"]
    ESTAT --> DC["Electric Circuits"]
    ESTAT --> MAG["Magnetism"]
    MAG --> EMIND["EM Induction"]
    EMIND --> EMWAVES["EM Waves"]
    EMWAVES --> LIGHT["Light & Optics"]
    EMWAVES --> WAVEOPTICS
    WORK --> RELATIVITY["Special Relativity"]
    RELATIVITY --> QUANTUM["Quantum Physics"]
    QUANTUM --> ATOMIC["Atomic Physics"]
    ATOMIC --> NUCLEAR["Nuclear Physics"]
    NUCLEAR --> PARTICLE["Particle Physics"]
    ATOMIC --> ASTRO["Astrophysics"]
```
