---
tags: [overview, advanced-science, physics, mechanics, high-school, ipst]
---

# Mechanics — กลศาสตร์

> **Category:** Advanced Physics (ม.4, ว301)
> **Covering:** Measurement, Kinematics, Dynamics, Work/Energy, Momentum, Rotation
> **Foundation for:** All of physics — mechanics is the first and most fundamental branch

## Overview

Mechanics is the study of **motion and its causes**. For Thai วิทย์-คณิต students, this is the first physics course (ม.4, ว301) — and the most important. Newton's laws, conservation of energy and momentum, and rotational motion form the conceptual toolkit that every other physics topic builds upon.

---

## Topic Areas

| # | Topic | Thai | Key Content |
|---|---|---|---|
| 01 | [[01_Measurement_and_Scientific_Method|Measurement and Scientific Method]] | การวัดและวิธีทางวิทยาศาสตร์ | SI units, significant figures, error analysis |
| 02 | [[02_Kinematics|Kinematics]] | จลนศาสตร์ | Displacement, velocity, acceleration, projectile motion, free fall |
| 03 | [[03_Dynamics|Dynamics]] | พลศาสตร์ | Newton's 3 laws, friction, inclined planes, circular motion |
| 04 | [[04_Work_Energy_and_Power|Work, Energy, and Power]] | งาน พลังงาน และกำลัง | KE, PE, work-energy theorem, conservation, power |
| 05 | [[05_Momentum|Momentum]] | โมเมนตัม | Impulse, conservation, elastic/inelastic collisions |
| 06 | [[06_Rotational_Motion|Rotational Motion]] | การเคลื่อนที่แบบหมุน | Torque, angular velocity, moment of inertia, angular momentum |

---

## Key Formulas

| Concept | Formula |
|---|---|
| **Kinematics** | $$v = u + at, \quad s = ut + \tfrac{1}{2}at^2, \quad v^2 = u^2 + 2as$$ |
| **Newton's 2nd Law** | $$F = ma$$ (or $$\Sigma F = ma$$) |
| **Friction** | $$f = \mu N$$ |
| **Kinetic Energy** | $$KE = \tfrac{1}{2}mv^2$$ |
| **Potential Energy** | $$PE = mgh$$ (gravitational) |
| **Work-Energy** | $$W = \Delta KE + \Delta PE$$ |
| **Momentum** | $$p = mv, \quad \Sigma p_i = \Sigma p_f$$ (conservation) |
| **Torque** | $$\tau = r \times F = I\alpha$$ |
| **Angular momentum** | $$L = I\omega$$ |

### The Big Four Kinematic Equations (constant acceleration)

| # | Equation | Missing |
|---|---|---|
| 1 | $$v = v_0 + at$$ | s |
| 2 | $$s = v_0t + \tfrac{1}{2}at^2$$ | v |
| 3 | $$v^2 = v_0^2 + 2as$$ | t |
| 4 | $$s = \tfrac{1}{2}(v_0 + v)t$$ | a |

---

## Newton's Three Laws

| Law | Statement | Thai |
|---|---|---|
| **1st (Inertia)** | Object at rest stays at rest; object in motion stays in motion unless acted upon by a net force | กฎของความเฉื่อย |
| **2nd (F=ma)** | Acceleration is proportional to net force and inversely proportional to mass | กฎของแรง |
| **3rd (Action-Reaction)** | For every action, there is an equal and opposite reaction | กฎของแรงกิริยา-ปฏิกิริยา |

---

## Key Thai Terminology

| Thai | English |
|---|---|
| การกระจัด | Displacement |
| ความเร็ว / ความเร่ง | Velocity / Acceleration |
| แรงเสียดทาน | Friction |
| งาน | Work |
| พลังงานจลน์ / พลังงานศักย์ | Kinetic / Potential energy |
| การชนแบบยืดหยุ่น / ไม่ยืดหยุ่น | Elastic / Inelastic collision |
| ทอร์ก | Torque |
| โมเมนต์ความเฉื่อย | Moment of inertia |

---

## Exam Relevance

| Exam | Weight |
|---|---|
| **A-Level Physics** | ~25-30% — Mechanics is the largest section |
| **PAT2** | ~20% — Mechanics problems are common |

---

## Cross-Links

- [[01_Physics - Overview|← Back to Physics]]
- [[02_Thermodynamics_and_Waves_-_Overview|06 Thermodynamics and Waves]]
- [[03_Electricity_and_Magnetism_-_Overview|07 Electricity and Magnetism]]
- [[04_Modern_Physics_-_Overview|08 Modern Physics]]
