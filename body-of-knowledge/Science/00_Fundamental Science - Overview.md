---
tags: [overview, fundamental-science, math-sci, primary, lower-secondary, ipst]
---
...
```mermaid
flowchart TD
    PROC["Scientific Method"] -.->|"process skills"| ALL["All Areas"]
    
    subgraph LIFE["🧬 Life Science"]
        LIVING["Living Things"] --> CELLS["Cells"]
        CELLS --> HUMAN["Human Body"]
        CELLS --> PLANTS["Plants"]
        LIVING --> ANIMALS["Animals"]
        LIVING --> ECOSYS["Ecosystems"]
        LIVING --> HEREDITY["Heredity & Evolution"]
    end

    subgraph CHEM["⚗️ Chemistry"]
        MATTER["Matter"] --> ATOMS["Atoms & Elements"]
        ATOMS --> REACTIONS["Chemical Reactions"]
        REACTIONS --> ACIDS["Acids & Bases"]
    end

    subgraph PHYS["⚛️ Physics"]
        FORCES["Forces & Motion"] --> ENERGY["Energy"]
        ENERGY --> HEAT["Heat & Temp"]
        ENERGY --> LIGHT["Light"]
        ENERGY --> SOUND["Sound"]
        FORCES --> ELEC["Electricity & Magnetism"]
    end

    subgraph EARTH["🌍 Earth & Space"]
        ROCKS["Rocks & Minerals"] --> WEATHER["Weather"]
        WEATHER --> SOLAR["Solar System"]
    end

    CELLS --> MATTER
    PLANTS --> ENERGY
    HEAT --> WEATHER
    ELEC --> MAGNETS["Magnets"]
    ECOSYS --> WEATHER
```
