---
tags: [overview, mathematics, math-sci, high-school, ipst]
---
...
```mermaid
flowchart TD
    FOUND["Sets, Logic, Numbers"] --> FUNC["Functions"]
    FUNC --> TRIG["Trigonometry"]
    FUNC --> SEQ["Sequences & Series"]
    TRIG --> LIMIT["Limits"]
    SEQ --> LIMIT
    LIMIT --> DIFF["Differentiation"]
    DIFF --> INT["Integration"]
    FOUND --> COMPLEX["Complex Numbers"]
    COMPLEX --> MAT["Matrices"]
    FOUND --> ANALGEO["Analytic Geometry"]
    MAT --> VEC["Vectors"]
    FUNC --> EXPLOG["Exp & Log"]
    LIMIT --> DIFFEQ["Differential Equations"]
    INT --> DIFFEQ
    FOUND --> DISC["Discrete Math"]
    DISC --> PROOF["Math Reasoning"]
    FUNC --> STAT["Probability & Stats"]
    LIMIT --> STAT
    STAT --> PROBDIST["Probability Distributions"]
    FUNC --> LINPROG["Linear Programming"]
```
