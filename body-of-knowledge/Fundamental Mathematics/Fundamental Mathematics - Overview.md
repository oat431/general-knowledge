---
tags: [overview, fundamental-mathematics, math-sci, primary, lower-secondary, ipst]
---
...
```mermaid
flowchart TD
    NUM["Numbers & Numeration"] --> ARITH["Arithmetic Operations"]
    ARITH --> FACTORS["Factors & Multiples"]
    ARITH --> INTEGERS["Integers"]
    ARITH --> FRACTIONS["Fractions"]
    ARITH --> DECIMALS["Decimals"]
    FRACTIONS --> RATIONAL["Rational Numbers"]
    DECIMALS --> RATIONAL
    FRACTIONS --> PERCENT["Percentages"]
    DECIMALS --> PERCENT
    ARITH --> RATIO["Ratios & Proportions"]
    PERCENT --> RATIO
    ARITH --> PATTERNS["Patterns"]
    PATTERNS --> ALGEBRA["Basic Algebra"]
    ALGEBRA --> EQUATIONS["Equations"]
    NUM --> GEOM["Geometry"]
    GEOM --> MEASURE["Measurement"]
    MEASURE --> AREA["Area & Perimeter"]
    AREA --> VOLUME["Volume & Surface Area"]
    GEOM --> COORD["Coordinate Plane"]
    COORD --> ALGEBRA
    ARITH --> SETS["Sets"]
    ARITH --> STATS["Statistics"]
    STATS --> PROB["Probability"]
    ARITH --> PROCESSES["Math Processes"]
    ALGEBRA --> PROCESSES
```
